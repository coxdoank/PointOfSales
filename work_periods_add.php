...
// opening balance
$qbal = "select res.OPENING_BALANCE from restaurant res";
$qbal = $connection -> query($qbal);
$rows = $qbal -> fetch_array();

//id transaksi 
$query = $connection -> query("select count(*) as idtrans from transaction trs where 
					trs.TERMINAL = '$terminal'");
$count = $query -> fetch_array();
$idtrans = $count['idtrans'] + 1;

if(isset($_POST['btn_add'])){
$txtterminal 	= $_POST['txtterminal'];
$txttrans_date  = $_POST['txttrans_date'];
$txtbalance 	= $_POST['txtbalance'];
$txtuser 		= $_POST['txtuser'];

	if($txtuser == ''){
	echo "<script>alert('Please select assignee user');window.history.back(-1)</script>";
	}else{
	$table_name = "transaction";
	$form_data = array(
				'ID_TRANSACTION' => $idtrans,
				'TERMINAL' => $txtterminal,
				'TRANS_DATE' => $txttrans_date,
				'CREATE_DATE' => date('Y-m-d H:i:s'),
				'OPENING_BALANCE' => $txtbalance,
				'ASSIGNED_USER' => $txtuser
				);
	...
	header("location:work_periods_print.php?id=$idtrans");

	// exec("D:\wamp64\bin\mysql\mysql5.7.31\bin\mysqldump -uroot --no-create-info --replace db_pos drawer_balance ticket ticket_item th_ticket th_ticket_item transaction > D:\BackupData_TransDate_$Date$Month$Year.sql");
	}
}
?>
...
<div class="navigation" id="navigation">
	<div class="navigationPanel" id="navigationPanel">
	  <table width="98%" align="center">
	    <tr>
	      <td class="label_dashboard_text">ADD WORK PERIODS</td>
        </tr>
	    <tr>
	      <td>&nbsp;</td>
        </tr>
	    ...
	    <tr>
	      <td><input name="txtterminal" type="text" class="input_text" id="txtterminal" value="<?php echo "$terminal" ?>" /></td>
        </tr>
	    <tr>
	      <td><span class="label_dashboard_text">Trans Date</span></td>
        </tr>
	    <tr>
	      <td><input name="txttrans_date" type="text" class="input_text" id="poptransdate" value="<?php echo "$YeadMonthDate" ?>" /></td>
        </tr>
	    <tr>
	      <td><span class="label_dashboard_text">Opening Balance</span></td>
        </tr>
	    <tr>
	      <td><input name="txtbalance" type="text" class="input_text" id="defaultKeypad" value="<?php echo "$rows[OPENING_BALANCE]" ?>" /></td>
        </tr>
	    <tr>
	      <td><span class="label_dashboard_text">Assignee User</span></td>
        </tr>
	    <tr>
	      <td><select name="txtuser" class="select_box" id="txtuser">
	        <option value="">- Choose -</option>
            <?php
			// user
			$quser = "select * from user usr
left join user_type ust on ust.ID = usr.USER_TYPE_ID
where ust.USER_TYPE <> 'ADMINISTRATOR' and usr.ACTIVE = '1'
order by ust.ID asc";
			$quser = $connection -> query($quser);
			while($rwusr = $quser -> fetch_array()){
			?>
            <option value="<?php echo "$rwusr[USER_ID]" ?>"><?php echo "$rwusr[FIRST_NAME] $rwusr[LAST_NAME] | $rwusr[USER_TYPE]" ?></option><?php } ?>
          </select></td>
        </tr>
      </table>
	</div>
            ...
            <div class="contentPanel" id="contentPanel">
              <input name="btn_add" type="submit" class="btn_ticket_action" id="submit5" value="Add Period" />
