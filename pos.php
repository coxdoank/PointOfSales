...
$qidtrs = mysqli_query($connection,"select ID_TRANSACTION,TRANS_DATE from transaction trs where trs.TERMINAL = '$terminal' and trs.FLAG = 'N'");
$rwtrs = mysqli_fetch_array($qidtrs);
$idtrans = $rwtrs['ID_TRANSACTION'];
$trsdate = explode("-",$rwtrs['TRANS_DATE']);

//baca no urut transaksi	
$query_noid = mysqli_query($connection,"select trm.NO_TRANSACTION as NOU_ID from terminal trm where trm.TERMINAL = '$terminal'");
$rowid = mysqli_fetch_array($query_noid);
$notrans = $rowid['NOU_ID'] + 1;
...
where 
ttm.NO_TRANSACTION = '$notrans' and ttm.TERMINAL = '$terminal'";
//print_r($q_display);
...
if(isset($_POST['btn_pay'])){
	...
	echo "";
/*	echo "<script>alert('Please Check Your Payment');window.history.back(-1)</script>"; */
...
/*	UpdateData($table_name, $form_data, "WHERE NO_TRANSACTION = '$notrans' and TERMINAL = '$terminal'");*/	
	...
	//backup transaksi berjalan
	exec("$paramMySQL\mysqldump -uroot --no-create-info --replace db_pos drawer_balance ticket ticket_item transaction > $paramApp\Transaksi_$trsdate[2]$trsdate[1]$trsdate[0].sql");
            ...
            <div class="contentPanel" id="contentPanel">
              <table width="100%">
                <tr>
                  ...
                  <?php echo"<a href=\"?id=$id&act=add&idm=$rwmn[ID_MENU_ITEM]\" class=\"menu_item\">$rwmn[MENU_NAME]</a> "; } ?></td>
                </tr>
              </table>
</div>
            <div class="clear"></div>
