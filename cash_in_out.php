...
//query view drawer balance
$query_view = mysqli_query($connection,"select * from
drawer_balance drw
left join transaction trs on trs.ID_TRANSACTION = drw.ID_TRANSACTION
left join user usr on usr.USER_ID = drw.UPDATE_BY
where drw.TERMINAL = '$terminal' and trs.TERMINAL = '$terminal' and trs.FLAG = 'N' order by drw.ID asc");
...
<div class="navigation" id="navigation">
  ...
  <table width="100%" border="0" cellpadding="3" cellspacing="0" class="table-model-01">
    <tr class="text_content">
      ...
      <td class="label_header"><strong>Account Name</strong></td>
      <td class="label_header"><strong>Cash Balance</strong></td>
      <td class="label_header"><strong>Update By</strong></td>
      </tr>
    <?php while($row = mysqli_fetch_array($query_view)){ ?>
    <tr class="text_content_02">
      <td><span class="label_header"><?php echo"$row[CREATE_DATE]" ?> Wib</span></td>
      <td><span class="label_header"><?php echo"$row[ACCOUNT_NAME]" ?></span></td>
      <td class="label_header"><?php echo number_format($row['CASH_BALANCE']) ?></td>
      <td class="label_header"><?php echo"$row[FIRST_NAME] $row[LAST_NAME]" ?></td>
      </tr>
    <?php } ?>
  </table>
  </div>
            ...
            <form action="" method="post">
              <input name="btn_start" type="button" class="btn_ticket_action" id="submit5" value="Add Cash Balance" onclick="window.location.href=('cash_in_out_add.php')"/>
